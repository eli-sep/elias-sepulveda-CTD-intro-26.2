...
const repeatArrow = integer => {
    for (let i = 0; i < integer; i++) {
        console.log("Q1 arrow function: Hello World!");
    }
}
repeatArrow(19);
...
//PUT YOUR CODE HERE
//added Math.abs & Math.sign to include negative numbers.

const pyramidCounting = (integer) => {
    let pyramid = 0;
    for (let i = 0; i <= Math.abs(integer); i++) {
        pyramid = pyramid + i;
    }
    return pyramid *= Math.sign(integer);
}
...
console.log("Q3 noVowels: ", noVowels("Supercalifragilisticexpialidocious"));
...
console.log("Q4 vowelCount: ", vowelCount("Supercalifragilisticexpialidocious"));
...
console.log("Q5 numOfOdds: ", numOfOdds(15));
...
function getElementAt(arr, inter) {
    if (inter < 0 || inter >= arr.length)
        return null;
    else return arr[inter];
}
...
function insertInArray(arr) {
    let newArr = arr.toSpliced(1, 0, 0);
    return newArr;
}
...
console.log("Q9 same: ", compareArrays(full, compare));
console.log("Q9 different: ", compareArrays(full, part));
console.log("Q9 partial: ", compareArrays(full, empty));
...
const skills = ["JavaScript", "HTML", "CSS", "I.T. Networking", "GitHub"];
